...
export const ControlExerciseList: React.FC = () => {
  ...
  useEffect(() => setAllExercises(allExercises || []), [allExercises, setAllExercises]);
    ...
    <div className="space-y-5">
      ...
      <div className={`grid gap-4 ${isMobile ? "grid-cols-2" : isDesktop ? "grid-cols-4" : "grid-cols-2 sm:grid-cols-3"}`}>
        {visibleExercises.map((ex) => {
          ...
          const cardBg = isMobile ? theme.lighter : theme.lighter; // subtle tinted bg on mobile
          const cardBorder = theme.translucentStrong;
